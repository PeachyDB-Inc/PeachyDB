...
<details>
  ...
  <ul>
    ...
    <li><b>1.g)</b> Secondary Indexes and Materialized Views add cost to writes. These are most pronounced in insert,delete. The SI refer to the primary table with the primary key, thus primary key modification is also expensive and discouraged. The more SI,MV you add to the table the slower the writes will be.</li><br>
     <li><b>1.h)</b> Long running reads tend to accumulte MVCC version data and have some impact on performance, thus long running reads/writes are discouraged. It is better to break up such transactions into smaller ones.</li><br>
  </ul>
</details>
...
<details>
  ...
  <b>16.b</b><code>peachydb_modify_server_stack</code>: to modify (add/remove/substitute nodes in) server AWS Cloudformation stack. Modifying server stack is a database transaction and hence must be submitted from a client instance in the client AWS cloudformation stack. Usage of this tool is explained <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Tools-ModifyServerStack.md">here</a>.<br><br>
