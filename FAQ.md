...
<details>
  ...
  <ul>
    ...
    <li><b>1.e)</b> After a Cluster Configuration change, vnode_gc will attempt to garbage collect vnodes from instances from which their ownership has been removed. This requires scan of the database and possibly a lot of writes to remove those data items. Thus immediately after the Cluster configuration change, the write performance will significantly degrade till the vnodes have been garbage collected. At the moment node add has this issue but node remove and node substitute don't have this issue, however that can change in future if the vnode allocation algorithms change.</li><br>
    ...
    <li><b>1.g)</b> Secondary Indexes and Materialized Views add cost to writes. These are most pronounced in insert,delete. The SI refer to the primary table with the primary key, thus primary key modification is also expensive and discouraged. The more SI,MV you add to the table the slower the writes will be.</li><br>
     <li><b>1.h)</b> Long running reads, running concurrently with writes, tend to accumulte MVCC version data and have some impact on performance, thus long running reads/writes are discouraged. It is better to break up such transactions into smaller ones. Running long running reads, at quiescent points, with low write activity, is probably better.</li><br>
  </ul>
</details>
...
<details>
  ...
  <br><br>
  If you believe you have short reads that are running into this issue, you can file an issue in GitHub. There is a chance that due to a race condition, an incorrect distributed <code>jnltxid</code> was assigned to a reader; this would be a bug that has to be addressed. In this case we would like to look at the peachdb.messglog from the support bundle to look for the messages of the form "mvccro has been garbage collected", this will provide us more details on where the race condition may be. You can paste these messages in the github issue you create to make it easier.
...
<details>
  ...
  <b>16.b</b><code>peachydb_modify_server_stack</code>: to modify (add/remove/substitute nodes in) server AWS Cloudformation stack. This tool must be executed on a client instance in the client AWS cloudformation stack. Usage of this tool is explained <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Tools-ModifyServerStack.md">here</a>.<br><br>
  <b>16.c</b><code>peachydb_status_tool</code>: to obtain support bundle and to monitor the health of server AWS Cloudformation stack. It is executed on the <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Step2--ControlUnit.md">Control Unit.</a> Usage of this tool is explained <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Tools-MonitorCluster.md">here</a>
  <br><br>
