...
<details>
  ...
  <ul>
    ...
    <li>1.d) After Cluster Configuration change (node add/delete/substitute) vnode_gc will attempt to garbage collect vnodes from instances from which their ownership has been removed. This requires scan of the database and possibly a lot of writes to remove those data items. Thus immediately after the Cluster configuration change, there will be an impact on the write performance.</li>
    <li>1.e) Write performance is also impacted a little by utilizing Partition Placement groups. However, for product clusters, using Placement groups is more or less required.</li>
