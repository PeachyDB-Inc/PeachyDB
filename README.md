...
- **29.g)** Before critisizing this prduct please be compassionate to the fact that we are a very small team with very limited resources. However, as we obtain more resouces, we expect to rapidly address majority of the issues and close the gap with the very best products. In the meantime the issues encountered should be pointed out by creating Github issues so we can proritize the requests.
