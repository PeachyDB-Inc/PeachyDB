...
- **6.g)** Transaction functionality is rather limited, this is because this is a first implementation. We will improve upon it when we get a chance. Long running reads/writes transactions tend to degrade behavior of all MVCC implementations. In our case we are simply not allowing
long running reads/writes to be transactional. Long running reads are allowed but are not transactional.Long running writes can not be a single transaction, as explained above in range-deletes and range-updates.
