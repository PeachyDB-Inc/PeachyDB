...
**NOTE**: The steps below are **OPTIONAL**, if the user wants to create their own S3 bucket, security-group and VPC and subnet they can follow these steps else they can allow the tools to auto-create these resources with auto generated names and or tags where applicable.  In order to auto-create security-group and subnet the user will have to provide a VPC-ID as that is not auto-generated. If these resources are auto-created they will be created only once for each specified AWS region and thereafter any subsequent invocations of the tools will lookup prior resource by names or tags in the AWS region and utilize those. So the very first attempt to create a stack will take extra time to create the below resources but thereafter the resources will be reutilized for any subsequent stack creation in the AWS region.

To get started its probably more convenient to auto-generate the security group, subnet (with auto generated CIDR block) with the specified VPC, as well as auto-generate the S3 bucket. The security group, subnet will be created with tags and any subsequent creation of stacks will attempt to lookup these resources by tags and reutilize them. Thus all stacks created in the AWS region will reutilize the same security group and subnet in the provided VPC.
