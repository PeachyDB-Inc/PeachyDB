...
**NOTE**: The steps below are **Optional**, if the user wants to provide their own S3 bucket name, security-group and VPC and subnet they can follow these steps else they can allow the tools to auto-create these resources with auto generated names and or tags where applicable.  In order to auto-create security-group and subnet the user will have to provide a VPC-ID as that is not auto-generated. If these resources are auto-created they will be created only once for each specified AWS region and thereafter any subsequent invocations of the tools will lookup prior resource by names or tags in the AWS region and utilize those. So the very first attempt to create a stack will incur an extra cost of creating the below resources but thereafter the resources will be reutilized for any subsequent stack creation in the AWS region.
