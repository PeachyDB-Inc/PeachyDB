...
1. peachydb_repair_cluster.py can be utilized to cleanup AWS state if a cluster configuration change did not complete as expected. There are two issues it resolves with an impaired AWS server stack. This tool only impacts AWS resources it has no impact on the underlying database. Since the tool does not interact with the product, it can be executed from the <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Step2--ControlUnit.md">Control Unit</a>.<br><br>
