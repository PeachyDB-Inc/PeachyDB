1. peachydb_repair_cluster.py can be utilized to cleanup AWS state if a cluster configuration change did not complete as expected. There are two issues it resolves with an impaired AWS server stack. This tool only impacts AWS resources it has no impact on the underlying database. <br><br>
