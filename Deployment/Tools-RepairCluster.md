...
**Prerequisite**: Before the below commands are executed we have to ascertain that there is no ongoing Cluster Configuration Change (add/remove/ssubstitute) at the database level. This is done with the cluster <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Tools-MonitorCluster.md"> status tool.</a> The status tool should clearly state that no CFG (cluster configuraton change) is ongoing. If a CFG is ongoing at the database level then we have to firstly allow that to either complete or cancel it, before we can start repairing the server stack. <br><br>
