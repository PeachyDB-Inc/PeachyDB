...
7. To **SUBSTTUTE** an AWS EC2 instance in a server stack this is what has to be done. If an AWS instance is live, then it can not be substituted, and the command will be rejected. After command returns follow step 3 above to check status of server stack.
```shell
  prompt> python3 peachydb_modify_server_stack.py
  1 - Stack Add Instance - add an EC2 instance to a server Stack
  2 - Stack Remove Instance - remove an EC2 instance from a server Stack
...
8. To **REJOIN** instance that exists in the AWS server stack this is the command, rejoin is effectively substituting the node with itself. If the node is currently live, as confirmed by the status tool, then rejoin will be rejected. After command returns follow step 3 to check status of server stack.
```shell
  prompt> python3 peachydb_modify_server_stack.py
  1 - Stack Add Instance - add an EC2 instance to a server Stack
  2 - Stack Remove Instance - remove an EC2 instance from a server Stack
