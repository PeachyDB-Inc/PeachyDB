...
4. It is best to perform such changes at quiescent points such as 12:00 AM Sunday night, with minimal write activty. Even with a large database that requires transfer of 100s of GB of data to the new node, the change should complete on the order of 20 minutes. However if you invoke the operation during heavy write activity, the same operation may take much longer and there is a real danger that it will run into out of disk space due to older versions. 
