...
3. When the below commands are executed, they submit the operation to the AWS infrastructure as well as to peachydb. In case of add and substitute, the AWS change is submitted first and takes some time to be completed at AWS and then it is submitted to peachydb. In case of remove the operation is first submitted to peachydb and once that is complete the instance is removed from AWS. <br><br>
   ...
   3.c) When running the below commands do **NOT** hit Ctrl-C. The command has already been submitted to AWS and it is only waiting to confirm the completion of the task. Hitting Ctrl-C will leave the AWS stack inconsistent with the underlying database and the stack will have to be <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Tools-RepairCluster.md"> repaired</a>.<br>
