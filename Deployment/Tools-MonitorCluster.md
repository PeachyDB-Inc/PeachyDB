...
4. To check liveness of nodes in the server cluster this is the command, it is recommended that liveness be sampled every 2 minutes.
   ...
   leaderid is the nodeid of the current leader. The ipaddresses of each node are listed so that they can be corelated with the output of <a href="https://github.com/PeachyDB-Inc/PeachyDB/blob/main/Deployment/Tools-ModifyServerStack.md"> stack description</a>.
