...
5) To check space utilization on the members of the server cluster utilize the following command, it is recommended that space utilization
   ...
   ```
   The command output displays for each node the amount of SSD storage space taken up by the database, the logs and the total capacity of the instance as well as % utilization of storage space. The space is displayed in MB (megabyte) or GB (gigabyte).
