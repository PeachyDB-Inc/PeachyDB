...
<details>
  <summary><b>1. Write performance tanked, what happened?</b></summary>
  <br>
  Writes in this database are subject to the following constraints:
  <ul>
    <li><b>1.a)</b> Each write will respond to the client only after all live servers which involve that write have responded to the leader node. Thus, a server node will slow down the writes that involve that server.</li>
    ...
    <li><b>1.c)</b> Cluster Configuration change (node add/delete/substitute) is ongoing. During this change, we have to deal with long-running read transactions which accumulate MVCC versions; to mitigate this, we throttle the writes significantly.</li>
    <li><b>1.d)</b> Write performance is also impacted a little by utilizing Partition Placement groups. However, for product clusters, using Placement groups is more or less required.</li>
    <li><b>1.e)</b> A small performance loss may also be incurred (if the database is small enough to fit in cache) due to requirements of the Raft consensus protocol.</li>
  </ul>
</details>

<details>
  <summary><b>2. The new node being added to the cluster appears to be hung, what should I do?</b></summary>
  <br>
...
<details>
  <summary><b>3. An AWS EC2 server stack instance died, what should I do?</b></summary>
  <br>
...
<details>
  <summary><b>4. How do I obtain a support bundle?</b></summary>
  <br>
...
<details>
  <summary><b>5. How do I monitor the server cluster?</b></summary>
  <br>
...
peachydb_status_tool -utilization &lt;stack-name&gt;</code></pre>
  ...
  <ul>
    <li><b>5.a)</b> Collect the support bundle and look at the logs: <code>peachdb.messglog*</code>, <code>perf.peachdb.messglog*</code>, <code>bulk.peachdb.messglog*</code></li>
    <li><b>5.b)</b> Generally, it is best not to SSH into the server instances. However, if a node is not responsive, you suspect some kind of issue, and you want to make a quick check, you can:
      <ul>
...
<details>
  <summary><b>6. I get "mvccro garbage collected" or "stale handle", what should I do?</b></summary>
  <br>
...
<details>
  <summary><b>7. How do I determine if clients or the servers are the bottleneck?</b></summary>
  <br>
...
<details>
  <summary><b>8. Why does mpstat on the client show uneven distribution of CPU utilization?</b></summary>
  <br>
