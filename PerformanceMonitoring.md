...
- If the CPUs on the client are underutilized, even after increasing threads/fibers, as determined by mpstat, most likely this is because the fibers/threads are spending that time waiting on response packets from the servers rather than doing any processing. In this case there may not be enough processing power available on the servers. In this case you can try a weaker client instance with more than the default number of fibers/threads to reduce costs and possibly attain similar performance from the servers. Allthough client costs may be a small fraction of the server costs.
