...
In order to figure out any performance related issues, you can obtain a performance bundle from the specific servers. There you will find a file perf.peachdb.messglog. This file contains some statistics on the database operation. Below is the explanation of numerous counters available. When collecting stats from any of the servers note that the stats are reported every 10 minutes (as of this writing, but you can easily check from the log file if it has been made more frequent), so there is not much to be gained by collecting stats more frequently and it will only add additional burden on the servers to keep returning these files to the user.
...
- **exec-calls**: Percentage of time spent in execution.
  - `exec-call = rpc-wait + rpc-callback`
...
- During testing phase, utilize 'mpstat' in a separate shell to monitor CPU utilization on the client machine.
...
- By default, the tests provided utilize a high number of fibers/threads per client so that the client is not underutilized.
...
- If servers are closed to max cpu utilization then larger server clusters are needed, if the client is close to max cpu utilization then likely more clients are needed.
