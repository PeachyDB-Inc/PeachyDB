...
- ___NOTE___: If increasing the client threads does not improve CPU utilization (particularly noticeable when the data is still served from cache), then most likely the AWS instance is not strong enough and does not have enough compute resources to run the database query execution threads without context switching. Utilizing a stronger instance can make a difference.
