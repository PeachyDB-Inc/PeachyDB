...
_IMPORTANT:_ Each AWS EC2 Instance must run only a single client driver. Multiple Client drivers running on a single AWS EC2 Instance is not supported and it could lead to all kinds of issues. The Client driver is multi-threaded.
