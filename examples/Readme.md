...
8. Try to utilize separate client driver threads/fibers for read queries and write queries. Since the write throughput is based upon completely different critera to read throughput, this will make it easier to investigate performance issues. For eg, each write has to be applied on as many nodes as needed to satisfy the replication factor, on the other hand reads only need to return results for the query from any one of the nodes that has the data. This alone would suggest that the cluster wide read throughput is probably higher than the cluster wide write throughput. Writes have additional transactional burden and are otherwise also slower than reads on cached data. Recompile the client with the script provided.
